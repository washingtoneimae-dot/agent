...
async def create_review(
    ...
    txn_result = await db.execute(
        select(Transaction).where(
            ...
            Transaction.file_id == req.file_id,
        )
    )
    has_downloaded = txn_result.scalar_one_or_none() is not None
    ...
    review = Rating(
        ...
        paired_file_ids=req.paired_file_ids,
        verified_download=has_downloaded,
    )
    db.add(review)
    await db.commit()
    await db.refresh(review)

    # Auto-trigger summary regeneration after each new review
...
async def get_summary(file_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(ReviewSummary).where(ReviewSummary.file_id == file_id)
    )
    summary = result.scalar_one_or_none()
    if not summary:
        return ReviewSummaryResponse(
            file_id=file_id,
            summary_json={"note": "Not enough reviews yet"},
            total_reviews=0,
            avg_score=0.0,
            generated_at=None,
            updated_at=None,
...
async def store_summary(file_id: int, payload: dict, db: AsyncSession = Depends(get_db)):
    """Store an externally generated review summary (webhook-compatible)."""
    summary = await _save_summary(file_id, payload, db)
    return {"success": True}
...
async def _regenerate_summary(file_id: int, db: AsyncSession):
    """Fetch reviews for a file, call AI, save the result."""
    # Get reviews
    review_result = await db.execute(
        select(Rating).where(Rating.file_id == file_id).limit(50)
    )
    ...
    review_data = [
        {
            ...
            "agent": r.agent_used or "unknown",
            "use_case": r.use_case or "unknown",
            "synthesis_rounds": r.synthesis_rounds or "unknown",
        }
        for r in reviews
    ]
    ...
    summary_data = await generate_review_summary(
        title=file.title,
        category=file.category.value if hasattr(file.category, 'value') else str(file.category),
        reviews=review_data,
    )
